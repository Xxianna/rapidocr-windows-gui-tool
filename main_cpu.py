...
# pyinstaller -D -w -i"C:\prj\ai\ocr\OCR-favicons-20253141659\favicon-192x192.png" main.py -n myocr.exe
####################给pyinstaller看的依赖#####################
import argparse
...
# import openvino.runtime as ov666
...
def draw_ocr_result(img_path):
    ...
    result, elapse = engine(str(img_path))  # 需要替换engine为你实际使用的函数
    for line in result:
        vertices = line[0]
        text = line[1]
        confidence = line[2]
...
mydet = os.path.join(current_directory, det_model_path)
myrec = os.path.join(current_directory, rec_model_path)

engine = RapidOCR(rec_model_path=myrec, det_model_path=mydet)
